...
class Paper:
    ...
    def tex_title(self) -> str:
        ...
        tex_title = tex_title.replace(r"0.5 - 1.4", r"0.5 to 1.4")

        tex_title = tex_title.replace(
            r"1.4 M<SUB>⊙</SUB>", r"\( \qty{1.4}{\solarmass} \)"
        )
        tex_title = tex_title.replace(
            r"1.4 M<SUB>sol</SUB>", r"\( \qty{1.4}{\solarmass} \)"
        )

        tex_title = tex_title.replace(r"Thorne─", r"Thorne--")
