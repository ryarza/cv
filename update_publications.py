...
class Paper:
    ...
    def tex_title(self) -> str:
        ...
        if match is not None:
            tex_title = self.title.replace(match.group(), rf"{match.group()[:-1]}\@:")
        ...
        if match is not None:
            tex_title = self.title.replace(match.group(), rf"``{match.group()[1:-1]}''")
    ...
    def author_string(self) -> str:
        ...
        my_position = [
            idx for idx, author in enumerate(self.authors) if author == MY_NAME
        ][0]
        ...
        if len(self.authors) == MAX_AUTHORS:
            temp_authors[-1] = rf"\& {self.authors[-1]}"

        if my_position > MAX_AUTHORS - 1:
            author_string = (
                ", ".join(self.authors[: MAX_AUTHORS - 1])
                + r", et al., incl.\ \textbf{"
                + MY_NAME
                + "}"
            )
